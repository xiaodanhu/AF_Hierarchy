...
class ANETdetection(object):
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        """
        # Filter ground truth to include only video-ids present in predictions
        gt_video_ids = preds['video-id'].unique()
        self.ground_truth = self.ground_truth[self.ground_truth['video-id'].isin(gt_video_ids)]
        self.activity_index = {j: j for i, j in enumerate(sorted(self.ground_truth['label'].unique()))}
        ...
        for i, cidx in enumerate(self.activity_index.values()):
            ap[:,i] = results[i] # ap[:,cidx] = results[i]
    ...
    def wrapper_compute_topkx_recall(self, preds):
        ...
        for i, cidx in enumerate(self.activity_index.values()):
            recall[...,i] = results[i] # recall[...,cidx] = results[i]
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if verbose:
            ...
            with open('./log.txt', 'a') as the_file:
                ...
                the_file.write('\n')
                the_file.write('Average mAP: {:>4.2f} (%)'.format(average_mAP*100))
                the_file.write('\n')

        # return the results
        return mAP, average_mAP, mRecall, self.ap
