...
def get_frame_path(rgb_root, video_id, frame_idx, total_frames):
    """ActivityNet version: always use 7-digit zero-padding to match
    the existing pre-extracted RGB frames."""
    num_digits = max(len(str(total_frames)), 7)
    frame_dir = os.path.join(rgb_root, video_id)
    frame_filename = f"{frame_idx:0{num_digits}d}.jpg"
    return os.path.join(frame_dir, frame_filename)


def load_sliding_window_jpg(rgb_root, video_id, video_path, window_start_frame,
                            window_length, stride, total_frames, video_fps):
    """ActivityNet version of load_sliding_window_jpg with 7-digit padding."""
    frame_indices = []
    ...
    frame_paths = [get_frame_path(rgb_root, video_id, idx, total_frames) for idx in frame_indices]
...
@register_dataset("activitynet_slide")
class ActivityNetSlideDataset(Dataset):
    """
    ActivityNet v1.3 dataset with SLIDING WINDOW approach.
    Mirrors Thumos14SlideDataset but adapted for ActivityNet's 200 classes.

    ActivityNet is a single-level action dataset (200 classes, no hierarchy).
    Each video is an untrimmed web video with one or more action instances.

    ActivityNet convention:
      - "training" subset = train split (~10k videos)
      - "validation" subset = test split (~5k videos, has annotations)
      - "testing" subset = challenge test set (no annotations, not used)

    Frames are extracted on-the-fly at sample_stride=16 and cached as JPG
    under {data_root}/RGB/{video_id}/{frame_idx:06d}.jpg.

    Annotation format (JSONL):
    {
      "video": "<video_id>",
      "span": ["<0.0 seconds>", "<duration seconds>"],
      "raw_value": [{"action_id": "cNNN", "span": ["<start>", "<end>"]}],
      "new_value": [{"action_id": "cNNN", "span": ["<start>", "<end>"]}]
    }

    Since videos are untrimmed and span=[0, duration], raw_value == new_value.
    """
    def __init__(
        self,
        is_training,
        ...
        **kwargs
        ...
        # ActivityNet data root
        self.data_root = '/data3/xiaodan8/Activitynet'

        self.use_raw_video = use_raw_video
        self.video_dir = os.path.join(self.data_root, 'video')
        print(f"[ActivityNetSlide] Using videos from {self.video_dir}")
        print(f"[ActivityNetSlide] Annotations: {'raw_value' if use_raw_video else 'new_value'}")

        # JPG frame caching
        self.load_jpg = load_jpg
        self.rgb_root = os.path.join(self.data_root, 'RGB')
        if self.load_jpg:
            print(f"[ActivityNetSlide] JPG caching ENABLED: {self.rgb_root}")
        ...
        print(f"[ActivityNetSlide] {len(label_dict)} action classes in annotations, "
        ...
        self.db_attributes = {
            'dataset_name': 'activitynet_slide',
    ...
    def _create_sliding_windows_cached(self, dict_db):
        ...
        for idx, video_item in enumerate(video_items):
            ...
            # ActivityNet video files have v_ prefix
            video_path = find_video_file(self.video_dir, f'v_{video_name}')
            ...
            if video_fps is None or video_fps == 0 or raw_total_frames == 0:
                # Try to estimate from annotation span
                span_end = video_item.get('instance_span_end')
                if span_end and span_end > 0:
                    video_fps = 30.0
                    raw_duration = span_end
                    raw_total_frames = int(span_end * video_fps)
                    metadata_cache[cache_key] = {
                        'fps': video_fps,
                        'total_frames': raw_total_frames,
                        'duration': raw_duration
    ...
    def _load_json_db(self, json_file):
        ...
        ActivityNet format: single-level, no activity/phrase grouping.
        raw_value/new_value is a flat list of {"action_id": ..., "span": [...]}.
        """
        json_db = {}
        ...
        if self.label_dict is None:
            ...
            for key, value in json_db.items():
                actions = value.get(annotation_key, [])
        ...
        for key, value in json_db.items():
            ...
            # ActivityNet: flat action list (no activity grouping)
            segments, labels = [], []
            for action in actions:
                span = action['span']
