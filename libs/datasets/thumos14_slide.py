...
@register_dataset("thumos14_slide")
class Thumos14SlideDataset(Dataset):
    """
    THUMOS14 dataset with SLIDING WINDOW approach.
    Mirrors FineGymSlideDataset but adapted for THUMOS14's single-level annotation.

    THUMOS14 is a single-level action dataset (20 classes, no hierarchy).
    Each video is an untrimmed sports video with multiple action instances.

    THUMOS14 convention:
      - "Validation" subset = training set (200 videos)
      - "Test" subset = test set (212 videos)

    Annotation format (JSONL):
    {
      "video": "<video_name>",
      "span": ["<0.0 seconds>", "<duration seconds>"],
      "raw_value": [{"action_id": "cNN", "span": ["<start>", "<end>"]}],
      "new_value": [{"action_id": "cNN", "span": ["<start>", "<end>"]}]
    }
    ...
    def __init__(
        ...
        **kwargs
        ...
        print(f"[Thumos14Slide] Using videos from {self.video_dir}")
        print(f"[Thumos14Slide] Annotations: {'raw_value' if use_raw_video else 'new_value'}")

        # JPG frame caching
        self.load_jpg = load_jpg
        self.rgb_root = os.path.join(self.data_root, 'RGB')
        if self.load_jpg:
            print(f"[Thumos14Slide] JPG caching ENABLED: {self.rgb_root}")
        ...
        print(f"[Thumos14Slide] {len(label_dict)} action classes in annotations, "
    ...
    def _load_json_db(self, json_file):
        ...
        THUMOS14 format: single-level, no activity/phrase grouping.
        ...
        for key, value in json_db.items():
            ...
            # THUMOS14: flat action list (no activity grouping)
            segments, labels = [], []
            for action in actions:
                span = action['span']
