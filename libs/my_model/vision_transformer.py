...
from torchvision.models import ViT_B_16_Weights, ViT_B_32_Weights,ViT_H_14_Weights, ViT_L_16_Weights, ViT_L_32_Weights, Weights, WeightsEnum
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        # self.layers = SwinModel.from_pretrained("microsoft/swin-tiny-patch4-window7-224")
        self.ln = norm_layer(hidden_dim)
        self.prompt_dropouts = [nn.Dropout(0.1) for i in range(num_layers)]

        scale_factor = 2
        kernel_size, stride, padding = scale_factor + 1, scale_factor, (scale_factor + 1)//2
    ...
    def forward(self, input: torch.Tensor, prompts: Optional[torch.Tensor] = None):
        ...
        for i in range(len(self.layers)):
            if prompts is not None:
                ...
                x = self.layers[i](x)
            x = self.ln(x)
        return x # [x[:, 0].reshape(2, -1, self.hid_dim).permute(0,2,1)]
